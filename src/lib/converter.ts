...
// Converter
export const convertBalanceToWei = (strValue: string, iDecimal: number = 18, options?: {
  isFormat?: boolean
}) => {
  strValue = '' + strValue
...
export const formatNumberBro = (number: string |number, mantissa: number = 4, isReturnNaN?: boolean, textNa?: string, trimMantissa: boolean = true) => {
  if (number !== 'null' && !(number === null) && !isNaN(number as number) && !(number === undefined) && number !== 'NaN' && number !== Infinity) {
...
export const calculateBigNumber = (arrValue: string[], method: 'add' | 'subtract'): string => {
  try {
    if (arrValue) {
      const sumAmount = arrValue.reduce(
        (preResult: bigDecimal, currentValue, currentIndex) => 
          currentIndex ? preResult[method](new bigDecimal(currentValue)) : new bigDecimal(currentValue), new bigDecimal(0)
      )
